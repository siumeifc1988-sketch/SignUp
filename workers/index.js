...
async function getTodayAttendance() {
  try {
    ...
    if (!res.ok) {
      if (res.status === 404) return { date: hkDate, empty: true };
      throw new Error(res.status);
    }
    const json = await res.json();
    return { date: hkDate, fields: json.fields || {} };
  } catch (e) {
...
function parseSlots(fields) {
  ...
  try {
    const map = fields?.slots?.mapValue?.fields;
    if (!map) return result;
    for (const k of Object.keys(result)) {
      const arr = map[k]?.arrayValue?.values || [];
      result[k] = arr.map(v => v.stringValue).filter(Boolean);
    }
  } catch {}
  return result;
}
    ...
    if (url.pathname === '/notify' && request.method === 'POST') {
      try {
        ...
        const embed = {
          ...
          fields: [
            { name: '球員', value: name, inline: true },
    ...
    if (request.method === 'POST') {
      ...
      if (interaction.type === 2) {
        ...
        if (cmd === '報名' || cmd === 'baoming') {
          ...
          const slots = parseSlots(att.fields);
          const total = Object.values(slots).flat().length;
          return Response.json({
            type: 4,
            data: {
              embeds: [{
                title: `⚽ 今日 ${att.date} 已有 ${total}人`,
                color: 0xA3FF12,
                fields: Object.entries(slots).map(([k,v]) => ({ name: k, value: v.join(', ') || '—', inline: false })),
                url: 'https://siumeifc1988-sketch.github.io/signup/'
              }]
            }
          });
        }
